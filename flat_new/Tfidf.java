...
public class Tfidf {
	...
	public void bagOfWordsQuery(String fileName) { 
	...
	private void bagOfWordsToTable(String fileName, Hashtable table) {
		...
		catch (IOException e) {System.out.println("Error: file " + fileName + "not found");}
		...
		while (s.hasNextLine()) {
			...
			double tf = 0;
			if (maxCount != 0)
				tf = Integer.parseInt(tokens[1]) / maxCount;			
			table.put(tokens[0], new Doub(tf*idf));
		}
	}
	
	public void processResearcher(String path) {
